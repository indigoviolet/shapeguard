...
- Added sg_noop() `from shapeguard import sg_noop as sg`
...
** DONE support no-op mode for sg()
...
** with reset_sg()
   context manager that resets known_dims at the end, so you can
   assign to dims and not have them contaminate future?
